#!/usr/bin/env python3
"""
Real Gemma 3:4b DSPy Tweet Optimizer

This version uses your actual Gemma 3:4b model with proper optimization
that follows the original app's rules (no emojis, no hashtags).
"""
...
# Page configuration
st.set_page_config(
    page_title=f"{PAGE_TITLE} - Real Gemma 3:4b",
    layout=PAGE_LAYOUT
)
...
# Real Gemma 3:4b model
GEMMA_MODEL = "ollama/gemma3:4b"
...
def initialize_session_state() -> None:
    ...
    # Set default model to Gemma 3:4b
    st.session_state.selected_model = GEMMA_MODEL
...
def render_sidebar_configuration() -> tuple:
    ...
    with st.sidebar:
        ...
        st.info("🤖 **Using Real Gemma 3:4b**\n\nThis app uses your actual Ollama Gemma 3:4b model for real AI optimization.")
        
        # Cache control
        use_cache = st.checkbox(
            "Enable DSPy Cache",
            value=st.session_state.use_cache,
            help="Enable DSPy's built-in caching to speed up repeated queries"
        )
        
        # Save settings if cache changed
    ...
    return GEMMA_MODEL, iterations, patience

def get_gemma_dspy_lm():
    """Get a DSPy LM instance for Gemma 3:4b."""
    try:
        lm = dspy.LM(
            model=GEMMA_MODEL,
        ...
        raise Exception(f"Failed to create Gemma LM: {str(e)}")
...
def main() -> None:
    ...
    <div style="background-color: #2a2a2a; padding: 1rem; border-radius: 0.5rem; margin: 1rem 0;">
        <h1 style="color: #00ff00; margin: 0;">🤖 Real Gemma 3:4b Tweet Optimizer</h1>
        <p style="margin: 0.5rem 0 0 0; color: #cccccc;">
            Using your actual Ollama Gemma 3:4b model for real AI optimization
        </p>
    </div>
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            gemma_models = [m for m in models if 'gemma3' in m.get('name', '')]
            if gemma_models:
                st.success(f"✅ Gemma 3:4b detected and ready! ({gemma_models[0]['name']})")
            else:
                st.warning("⚠️ Gemma 3:4b not found. Make sure it's installed:")
                st.code("ollama pull gemma3:4b")
        else:
            st.error("❌ Ollama not responding. Make sure it's running:")
            st.code("ollama serve")
    except Exception as e:
        st.warning(f"⚠️ Could not verify Gemma status: {str(e)}")
        st.info("💡 If you're sure Gemma 3:4b is installed, you can continue anyway.")
    
    # Sidebar configuration
    selected_model, iterations, patience = render_sidebar_configuration()
    
    # Initialize DSPy with Gemma
    try:
        # Configure cache settings
        dspy.settings.configure(cache=st.session_state.use_cache)
        
        # Get the Gemma LM
        gemma_lm = get_gemma_dspy_lm()
        dspy.configure(lm=gemma_lm)
        
        st.success("✅ DSPy initialized with Gemma 3:4b")
    except Exception as e:
        st.error(f"❌ Failed to initialize DSPy with Gemma: {str(e)}")
        st.info("💡 Make sure Ollama is running and Gemma 3:4b is installed:")
    ...
    if st.session_state.optimization_running and hasattr(st.session_state, 'optimizing_text'):
        # Get the LM for Gemma
        selected_lm = get_gemma_dspy_lm()
        ...
        try:
            # Run optimization with Gemma using dspy.context
