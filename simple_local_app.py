...
# Local-only models (no API keys required)
LOCAL_MODELS = {
    "Gemma 3 4B (Local)": "ollama/gemma3:4b",
    "Llama 3.2 3B (Local)": "ollama/llama3.2:3b",
    "Llama 3.2 1B (Local)": "ollama/llama3.2:1b",
...
def initialize_session_state() -> None:
    ...
    # Set default model to local Gemma if not set
    if not hasattr(st.session_state, 'selected_model') or not st.session_state.selected_model:
        st.session_state.selected_model = "ollama/gemma3:4b"
...
def render_sidebar_configuration() -> tuple:
    ...
    with st.sidebar:
        ...
        reverse_model_options = {v: k for k, v in LOCAL_MODELS.items()}
        current_model_name = reverse_model_options.get(st.session_state.selected_model, "Gemma 3 4B (Local)")
        
        # Handle case where current model is not available
        if current_model_name not in LOCAL_MODELS:
            current_model_name = "Gemma 3 4B (Local)"
            st.session_state.selected_model = LOCAL_MODELS[current_model_name]
        
        current_index = list(LOCAL_MODELS.keys()).index(current_model_name)
        ...
        new_model = LOCAL_MODELS[selected_model_name]
...
def get_local_dspy_lm(model_name: str):
    """Get a DSPy LM instance for local Ollama models."""
    try:
        ...
        raise Exception(f"Failed to create local LM: {str(e)}")
...
def main() -> None:
    ...
    try:
        ...
        # Get the local LM
        local_lm = get_local_dspy_lm(st.session_state.selected_model)
    ...
    if st.session_state.optimization_running and hasattr(st.session_state, 'optimizing_text'):
        ...
        selected_lm = get_local_dspy_lm(st.session_state.selected_model)
