#!/usr/bin/env python3
"""
Fixed Gemma 3:4b DSPy Tweet Optimizer

This version fixes the DSPy settings conflict and connection issues.
"""
...
# Page configuration
st.set_page_config(
    page_title=f"{PAGE_TITLE} - Fixed Gemma 3:4b",
    layout=PAGE_LAYOUT
)
...
def get_gemma_dspy_lm():
    """Get a DSPy LM instance for Gemma 3:4b (cached)."""
    try:
        lm = dspy.LM(
            model=GEMMA_MODEL,
...
def main() -> None:
    ...
    <div style="background-color: #2a2a2a; padding: 1rem; border-radius: 0.5rem; margin: 1rem 0;">
        <h1 style="color: #00ff00; margin: 0;">🤖 Fixed Gemma 3:4b Tweet Optimizer</h1>
    ...
    # Initialize DSPy with Gemma (without global configuration)
    try:
        # Get the Gemma LM (cached)
    ...
    if st.session_state.optimization_running and hasattr(st.session_state, 'optimizing_text'):
        ...
        try:
            # Run optimization with Gemma using dspy.context (this avoids the settings conflict)
